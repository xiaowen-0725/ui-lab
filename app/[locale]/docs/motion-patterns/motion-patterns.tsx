...
const patternsZh: Pattern[] = [
  {
    ...
    demo: "icon",
    ask: "让图标的动作贴合它真实的含义。",
    shape: "铃铛从顶部摆动,下载图标下坠,复制图标轻弹一下。",
    avoid: "不要给所有图标套用同一种通用弹跳。",
    code: `// Bell should swing from the top, not bounce.
hover: {
  rotate: [0, 14, -12, 8, -5, 0],
  transformOrigin: "top center",
  ...
  },
  {
    ...
    icon: Sparkles,
    demo: "reveal",
    ask: "让新内容缓缓浮现,而不是瞬间出现。",
    shape: "淡入、上移 12px,并缓慢去除模糊。",
    avoid: "不要对每个细小子元素都做错位动画,先让主体浮现。",
    code: `initial: { opacity: 0, y: 12, filter: "blur(6px)" }
animate: { opacity: 1, y: 0, filter: "blur(0px)" }
transition: { duration: 0.52, ease: [0.16, 1, 0.3, 1] }`,
  },
...
transition: {
  ...
  },
  {
    ...
    demo: "expand",
    ask: "先展开形状,再显示内容。",
    shape: "初始是一个紧凑的图标,在悬停、聚焦或点击时展开;形状移动后文字再淡入。",
    avoid: "不要让尺寸和文字同时进行动画。",
    code: `expanded = open || hovered || focused
...
// Shape moves first. Text follows.`,
  },
  {
    ...
    demo: "page",
    ask: "安静地切换视图。",
    shape: "旧视图快速退出,新视图伴随 4px 位移淡入。",
    avoid: "不要为小的标签切换使用大幅度的页面动效。",
    code: `exit: { opacity: 0, y: -4, transition: { duration: 0.12 } }
enter: { opacity: 1, y: 0, transition: { duration: 0.18 } }
initial: { opacity: 0, y: 4 }`,
  },
...
export function MotionPatterns() {
  ...
  const patterns = locale === "zh" ? patternsZh : patternsEn;
  ...
  return (
    <>
      ...
      >
        {active ? <PatternDetails pattern={active} locale={locale} /> : null}
      </BottomSheet>
    </>
  );
}
...
function PatternDetails({
  ...
  const rows = [
    { label: locale === "zh" ? "要求" : "Ask for", value: pattern.ask },
    { label: locale === "zh" ? "形态" : "Shape", value: pattern.shape },
    { label: locale === "zh" ? "避免" : "Avoid", value: pattern.avoid },
  ];
...
function LiveExample({
      ...
      >
        ...
        >
          <p className="font-display text-sm uppercase text-foreground">
            {locale === "zh" ? "新卡片" : "New card"}
          </p>
          <p className="mt-2 text-sm text-muted-foreground">
            {locale === "zh" ? "柔和浮现并上移。" : "Soft reveal with lift."}
          </p>
        </motion.div>
      </DemoFrame>
    );
  ...
  const tabs = [
    { key: "Preview", label: locale === "zh" ? "预览" : "Preview" },
    { key: "Code", label: locale === "zh" ? "代码" : "Code" },
  ];

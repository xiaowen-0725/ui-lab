...
1. **Reference Pack**：已声明的 calibration source 与 approved acceptance evidence。
2. **System Preset**：应用级视觉、技术栈与安全约束。
...
若 approved contract 没有定义某项，优先沿用所选 System/Recipe/source family 的默认值，并把需要用户决定的新视觉方向退回 `select`。不要引入“永远不用某种颜色/圆角/动画”一类个人审美绝对规则。

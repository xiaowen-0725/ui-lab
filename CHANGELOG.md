...
- **`agent-composer`(Agent 输入台)block**:AI agent 聊天输入台组件族,上下文 chips(悬停切换图标)+ 权限 chip + 分段推理力度滑杆的模型选择器 + 可变形发送/停止按钮,并带 ＋ 添加菜单与语音听写态(虚线声纹轨 + 计时 + 停止钮);最高推理力度档带渐变微光扫过、光晕呼吸与波纹扩散特效。

...
本目录记录 UI Lab 在独立、可重跑场景中的能力边界。目标不是制造一个可营销的总分，而是用同一套 rubric 区分已验证能力、证据不足和真实 blocker，持续发现 Skill、Catalog、System Preset、Recipe、CLI、视觉证据与消费项目之间的断点。
...
四个场景都按以下六个维度判断。只有存在可复核证据时才得分；`Insufficient evidence` 不得补写成通过。
...
| D | Parking adopt readiness | 真实消费项目的 frontend root、binding、保留契约、采用授权与落地前置条件 | `019fab74-ac33-7071-a7d9-da4765002db9` |
...
Static 分数、strict Audit `0 errors / 0 warnings`、构建成功或 E2E 全过，都不能替代 Visual 与 Human 结论。候选截图与自身比较也不能证明它忠实于 Codex 或其他 calibration source。
...
即使达到以上门槛，只要缺少 approved acceptance capture 或用户批准，Visual 仍是 `Insufficient evidence` / `Pending`，Human 仍是 `Pending`，Confirmed Manifest 继续阻断。

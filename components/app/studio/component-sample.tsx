...
export function ComponentSample({
  ...
  const style = {
    ...
    "--wb-surface-alpha": skin.vars["--wb-surface-alpha"],
    "--wb-blur": skin.vars["--wb-blur"],
    colorScheme: config.scheme,
    fontFamily: tokens.fontPairing.body,
  } as CSSProperties;

  const surfaceClass =
    "rounded-[var(--studio-radius)] border border-[var(--wb-border)] shadow-[var(--studio-shadow)]";
  const raisedSurfaceStyle: CSSProperties = {
    background:
      "color-mix(in srgb, var(--wb-surface-raised) var(--wb-surface-alpha), transparent)",
    backdropFilter: "blur(var(--wb-blur))",
    WebkitBackdropFilter: "blur(var(--wb-blur))",
  };
  ...
  return (
    <section
      ...
      className={cn(
        "relative overflow-hidden rounded-3xl border border-[var(--wb-border)] bg-background p-4 text-foreground sm:p-5",
    ...
    >
      {config.glass ? (
        <div
          ...
          style={{
            background:
              "radial-gradient(120% 120% at 20% 0%, color-mix(in srgb, var(--wb-accent) 22%, transparent), transparent 60%)",
          }}
        />
      ) : null}
        ...
        <div className="mt-4 grid gap-[var(--studio-space)] xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
          <div className="flex flex-col gap-[var(--studio-space)]">
            <div className="flex flex-wrap gap-[var(--studio-space)]">
              ...
              <button
                type="button"
                style={raisedSurfaceStyle}
                className="min-h-[var(--studio-row-height)] rounded-[var(--studio-radius)] border border-[var(--wb-border)] px-[var(--studio-space)] text-sm font-medium text-foreground"
              >
                {copy.secondary}
              </button>
            ...
            <select
              ...
              className="min-h-[var(--studio-row-height)] rounded-[var(--studio-radius)] border border-[var(--wb-border)] bg-[var(--wb-surface)] px-[var(--studio-space)] text-sm text-foreground outline-none focus:border-[var(--wb-accent)]"
            >
              <option>{copy.selectOptionA}</option>
              <option>{copy.selectOptionB}</option>
              <option>{copy.selectOptionC}</option>
            </select>
          ...
          <article style={raisedSurfaceStyle} className={cn(surfaceClass, "p-[var(--studio-space)]")}>
